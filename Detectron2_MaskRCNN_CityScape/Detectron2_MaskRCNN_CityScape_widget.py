...
# --------------------
class Detectron2_MaskRCNN_CityScapeWidget(PyCore.CProtocolTaskWidget):

    def __init__(self, param, parent):
        PyCore.CProtocolTaskWidget.__init__(self, parent)

        if param is None:
            self.parameters = processMod.Detectron2_MaskRCNN_CityScapeParam()
        else:
...
#--------------------
class Detectron2_MaskRCNN_CityScapeWidgetFactory(PyDataProcess.CWidgetFactory):

    def __init__(self):
        PyDataProcess.CWidgetFactory.__init__(self)
        # Set the name of the process -> it must be the same as the one declared in the process factory class
        self.name = "Detectron2_MaskRCNN_CityScape"

    def create(self, param):
        # Create widget object
        return Detectron2_MaskRCNN_CityScapeWidget(param, None)
