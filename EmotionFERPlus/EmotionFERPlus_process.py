...
#- Inherits PyCore.CProtocolTaskParam from Imageez API
#--------------------
class EmotionFERPlusProcessParam(PyCore.CProtocolTaskParam):
...
#- Inherits PyCore.CProtocolTask or derived from Imageez API
#--------------------
class EmotionFERPlusProcess(PyDataProcess.CImageProcess2d):
    ...
    def run(self):
        ...
        if input_graphics.isDataAvailable():
            ...
            for item in items:
                if item.getType() == PyCore.GraphicsItem.RECTANGLE:
                    x = int(item.x)
                    y = int(item.y)
                    w = int(item.width)
                    h = int(item.height)
...
#- Inherits PyDataProcess.CProcessFactory from Imageez API
#--------------------
class EmotionFERPlusProcessFactory(PyDataProcess.CProcessFactory):
